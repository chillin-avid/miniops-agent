...
- 基于服务、组件、故障现象和原因构建可审计的依赖图；模型结合精简节点目录提交实体入口，程序校验后扩展一至两跳，同时保持原始问题不被规范节点名改写。
...
| `knowledge_graph.py` | 节点目录生成、实体入口校验、别名兜底、一至两跳遍历和关系路径解释 |
| `knowledge_graph.json` | 服务、组件、故障现象、原因、手册节点及其关系 |
| `tools.py` | `search_runbooks` 与 `query_logs` 两个只读工具；检索问题和图谱入口参数彼此独立 |
